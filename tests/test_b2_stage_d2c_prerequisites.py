...
class TestProductionSafety(unittest.TestCase):
    ...
    def test_only_authorized_validation_modules_exist(self):
        ...
        setup invalidation and execution quality, and D-2C4 authorized
        exactly one more, ``envelope.py``, for deterministic validation
        identity/provenance. The check itself is unchanged and still fails
        on any UNapproved module. ``metrics.py`` stays forbidden --
        aggregation belongs to a later stage.
        """
