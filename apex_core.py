    ...
    if is_admin_user:
        ...
        with b5:
            if st.button("🔮 Forecaster", use_container_width=True, type="primary" if st.session_state["active_tab"] == "🔮 Forecaster" else "secondary"):
                ...
                st.rerun()
        with b6:
            if st.button("👑 MASTER ADMIN", use_container_width=True, type="secondary"):
                _set_public_view("admin")
    else:
...
def _set_public_view(view: str) -> None:
    """Navigate between real Streamlit routes while preserving session state."""
...
def bootstrap_runtime() -> tuple[str, str]:
    """Initialize shared CSS and background workers safely from any route."""
    inject_css()
