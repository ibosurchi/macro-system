...
# Hard spend guard: ordinary news/data churn can produce at most one paid batch per hour.
# A newly published High Impact actual may bypass this guard because it is market-critical.
_AI_BATCH_MIN_PAID_INTERVAL_SECONDS = 60 * 60
...
def _ai_batch_actual_signature(event_rows: list) -> str:
    """Fingerprint only published event actuals; used for the sole urgent cooldown bypass."""
    actuals = []
    for row in event_rows or []:
        ...
        if actual and actual not in {"—", "-", "N/A", "None"}:
            actuals.append((str(row.get("code", "")), actual))
    return hashlib.sha256(json.dumps(sorted(actuals), ensure_ascii=False).encode("utf-8")).hexdigest()
...
def _run_shared_ai_batch_once() -> None:
    ...
    try:
        ...
        actual_signature = _ai_batch_actual_signature(event_rows)
        state = _ai_batch_load_state()
        if state.get("signature") == signature and isinstance(state.get("result"), dict):
            return

        # Cost control: after the first successful shared result, ordinary headline,
        # ranking, description, forecast, or macro-snapshot churn is accumulated in
        # the background and may trigger at most ONE paid refresh per hour. Only a
        # genuinely new published High Impact Actual can bypass the hourly guard.
        has_result = isinstance(state.get("result"), dict) and bool(state.get("result"))
        if has_result:
            last_paid = float(state.get("updated_at", 0) or 0)
            prior_actual_sig = str(state.get("actual_signature", "") or "")
            urgent_actual_release = bool(actual_signature and actual_signature != prior_actual_sig)
            if not urgent_actual_release and time.time() - last_paid < _AI_BATCH_MIN_PAID_INTERVAL_SECONDS:
                return
