...
def page_catalyst_forecaster(fred_key: str, channel_name: str, auth_user: dict | None = None) -> None:
    ...
    with st.spinner("Synthesizing upcoming economic calendar, precursor FRED pipelines & correlated news..."):
        ...
        # Freeze pre-release predictions before any newly published Actual is ingested.
        # This prevents hindsight leakage and creates a real backtesting trail.
        for event in events:
            event_code = str(event.get("code", "")).strip()
            published_actual = _normalize_forex_factory_actual(event.get("actual_str", ""))
            saved_actual = str(actuals_cache.get(event_code, "")).strip()
            if event_code and not (published_actual or saved_actual):
                try:
                    pre_nowcast = compute_event_nowcast(event, fred_key, all_news, actual_override="")
                    _record_forecaster_snapshot(event, pre_nowcast, actual="")
                except Exception:
                    pass
        ...
        for event in events:
            code = str(event.get("code", "")).strip()
            actual = str(actuals_cache.get(code, "")).strip() or _normalize_forex_factory_actual(event.get("actual_str", ""))
            if code and actual:
                try:
                    _record_forecaster_snapshot(event, compute_event_nowcast(event, fred_key, all_news, actual_override=""), actual=actual)
                except Exception:
                    pass
